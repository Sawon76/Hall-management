  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (profile.role !== ROLES.STAFF) {
        setErrorMessage('Access denied: Not a staff account')
        await supabase.auth.signOut()
        return
      }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-900 to-blue-900 px-4 py-12">
      <div className="w-full max-w-md rounded-xl border border-white/15 bg-white/95 p-8 shadow-2xl backdrop-blur">
        <div className="mb-8 text-center">
          ...
          <span className="mt-3 inline-flex rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-blue-700">
            Staff Portal
          </span>
        </div>
